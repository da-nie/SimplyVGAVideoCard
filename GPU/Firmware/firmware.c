...
void SetColor(unsigned char y,unsigned char x,unsigned char color);//установить данные в видеопамяти
void PrintProgmem(unsigned char y,unsigned char x,unsigned char color,unsigned char bk_color,const char *string);//вывод текста из программной памяти в позицию
//----------------------------------------------------------------------------------------------------
...
int main(void)
{
 ...
 int y;
 InitAVR();
 //переключаем управление на вывод видео
 ...
 _delay_ms(5000);//ждём выхода на режим SD-карты
 int print_y=0;
 ...
 while(1)
 {
  //ищем первый попавшийся файл и отображаем его
...
//----------------------------------------------------------------------------------------------------
//установить данные в видеопамяти
//----------------------------------------------------------------------------------------------------
void SetColor(unsigned char y,unsigned char x,unsigned char color)
{
 color=0xff^color;//инвертируем цвета (так получилось из-за ключей на плате ЦАП)
 while(1)
 {
  if (BLANK_PIN&(1<<BLANK)) break;
 }
 OE_PORT|=1<<OE;//запрещаем вывод данных
 asm volatile ("nop"::);
 asm volatile ("nop"::);
 SELECT_PORT|=1<<SELECT;//переключаем управление на себя
 //устанавливаем адрес
 DATA_DDR=0xff;
 ROW_PORT=y;
 COL_PORT=x;
 ...
 //asm volatile ("nop"::);
 //asm volatile ("nop"::);
 WE_PORT&=0xff^(1<<WE);//даём сигнал записи
 //asm volatile ("nop"::);
 //asm volatile ("nop"::);
 //asm volatile ("nop"::);
 //asm volatile ("nop"::);
 WE_PORT|=1<<WE;//снимаем сигнал записи
 //asm volatile ("nop"::);
 //asm volatile ("nop"::);
 DATA_DDR=0;
 DATA_PORT=0;
 SELECT_PORT&=0xff^(1<<SELECT);//переключаем управление на видеокарту
 asm volatile ("nop"::);
 asm volatile ("nop"::);
 OE_PORT&=0xff^(1<<OE);//разрешаем вывод данных
}


//----------------------------------------------------------------------------------------------------
//вывод текста из программной памяти в позицию
//----------------------------------------------------------------------------------------------------
